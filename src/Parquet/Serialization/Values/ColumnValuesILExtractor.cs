...
namespace Parquet.Serialization.Values
{
   class ColumnValuesILExtractor : IColumnValuesExtractor
   {
      ...
      {
         ExtractColumns extract = GetExtractionDelegate(classType, schema, columns);
      ...
      private ExtractColumns GetExtractionDelegate(Type classType, Schema schema, List<IList> columns)
      {
         var key = new KeyValuePair<Type, Schema>(classType, schema);
         ...
         extractColumns = BuildExtractMethod(classType, schema, columns);
         TagToDelegate[key] = extractColumns;
         return extractColumns;
      }

      private ExtractColumns BuildExtractMethod(Type classType, Schema schema, List<IList> columns)
      {
